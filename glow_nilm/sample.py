...
采样/生成脚本
=============
从已训练的 Glow 模型中采样，生成与真实电器用电信号统计特性相近的样本。

用法：

    python sample.py --appliance kettle --n_samples 200 --temperature 0.9

生成结果保存在 results/<appliance>/samples.npy 及可视化图像。
"""
...
def main():
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"未找到模型权重: {ckpt_path}。请先运行 train.py 训练模型。"
        )
    ...
    # 采样
    print(f"[采样] 生成 {args.n_samples} 个样本，温度={temperature} ...")
    with torch.no_grad():
    ...
    print(f"[采样] 完成，样本形状: {samples.shape}")
